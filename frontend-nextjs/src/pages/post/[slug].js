...
export default function PostPage({ post, error }) {
  ...
  // Loading state (for client-side navigation)
  ...
  return (
    <>
      <SEO
        title={`${post.title} | RecursivAI`}
        description={post.summary || "AI-powered analysis of machine learning research"}
        ogTitle={post.title}
        ogDescription={post.summary}
        ogImage={post.image}
        ogUrl={`https://recursivai.vercel.app/post/${post.slug}`}
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    // Ensure all potentially undefined values are converted to null for JSON serialization
    const sanitizedPost = {
      ...post,
      ...
      ai_metadata: post.ai_metadata || null
    };
